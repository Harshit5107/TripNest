...
router.post('/login', async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      ...
      const isRoleAdmin = email.includes('admin') || email.includes('hbhalani');
      ...
      const [ins] = await db.query(
        ...
        [isRoleAdmin ? 'Admin Manager' : 'Demo Traveler', email, hashedPass, isRoleAdmin ? 'admin' : 'user', avatarUrl]
      );
